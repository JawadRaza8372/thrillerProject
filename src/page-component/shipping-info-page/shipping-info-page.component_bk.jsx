...
export const ShippingInfoPage = withRouter(({ history }) => {
  ...
  useEffect(() => {
    var items = JSON.parse(localStorage.getItem("cords"));
  ...
  useEffect(() => {
    try {
      var items = JSON.parse(localStorage.getItem("cords"));
      var shipName = localStorage.getItem("shipName");
      var shipLastName = localStorage.getItem("shipLastName");
      var shipPhone = localStorage.getItem("shipPhone");
  ...
  const getCordinates = async (address, rel) => {
    await Geocode.fromAddress(address).then(
      (response) => {
        ...
        localStorage.setItem("shipName", userCredentials.firstName);
        localStorage.setItem("shipLastName", userCredentials.lastName);
        localStorage.setItem("shipPhone", userCredentials.phoneNumber);
    ...
    await Geocode.fromAddress(address + ", " + city).then(
      (response) => {
        ...
        if (
          ...
          shippingData.firstName = localStorage.getItem("shipName");
          shippingData.lastName = localStorage.getItem("shipLastName");
          shippingData.phone = localStorage.getItem("shipPhone");
        }
        ...
        if (
          ...
          return;
        } else if (!validateAddress(shippingData.address)) {
          swal({
            title: "Error!",
            text: "Please enter the correct address",
            icon: "error",
          });
          return;
        } else if (lati === 0) {
